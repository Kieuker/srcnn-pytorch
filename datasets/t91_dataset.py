...
class T91Dataset(Dataset):
    def __init__(self, image_dir, transform = None, patch_size = 33, scale_factor = 3, color_mode = 'RGB', crop_gt_border_by_n=None):
        self.image_dir = image_dir
        self.image_names = os.listdir(image_dir)
        self.transform = transform if transform else transforms.RandomCrop((patch_size, patch_size)) # Set random crop as a default transform
    ...
    def __getitem__(self, idx):
        ...
        # Create low-resolution image by downsampling and upsampling the image(LR_image)
        LR_image = GT_image.resize((self.patch_size // self.scale_factor, self.patch_size // self.scale_factor))
